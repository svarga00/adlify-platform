...
-- ============================================
-- ▶▶ 016_client_users_safety_net.sql (PRE-MIGRATION)
-- ============================================
-- POZOR: ide ako PRVÁ napriek vyššiemu číslu, lebo všetky neskoršie
-- migrácie (011/012/013/014) majú RLS policies čo referenčujú
-- client_users a padali by ak by tabuľka neexistovala.
-- ============================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'client_users_client_id_fkey'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'client_users_user_id_fkey'
  ) THEN
...
-- Safety net: ADD COLUMN IF NOT EXISTS pre pred-existujúce tabuľky
